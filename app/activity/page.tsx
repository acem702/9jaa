      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-8 pb-32">
        ...
        <div className="mb-6 md:mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl md:text-4xl font-black text-slate-900">Activity Feed</h1>
          </div>
          <p className="text-sm md:text-lg text-slate-600 font-medium">Your recent market transactions and opinions</p>
        </div>
            ...
            {Object.entries(groupedByDate).map(([date, datePositions]) => (
              <div key={date}>
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <span className="text-xs font-bold text-slate-500 uppercase tracking-wider px-3 py-1 bg-slate-100 rounded-full">
                    {date}
                  </span>
                  <div className="h-px flex-1 bg-slate-200"></div>
                </div>
...
function ActivityCard({ position }: { position: Position }) {
  ...
  return (
    <Link href={`/market/${position.question.id}`}>
      <div className="bg-white rounded-2xl border border-slate-200 hover:border-blue-300 hover:shadow-lg transition-all p-4 md:p-5 group">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-start justify-between gap-2 mb-2">
              <div className="flex-1 min-w-0">
                ...
                <h3 className="text-sm md:text-base font-black text-slate-900 line-clamp-2 group-hover:text-blue-600 transition-colors">
            ...
            <div className="grid grid-cols-3 gap-3 mt-3 pt-3 border-t border-slate-100">
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Shares</p>
                <p className="text-sm font-black text-slate-900">{(position.shares || 0).toLocaleString(undefined, { maximumFractionDigits: 2 })}</p>
              </div>
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Value</p>
                <p className="text-sm font-black text-slate-900">{(position.current_value || 0).toLocaleString(undefined, { maximumFractionDigits: 2 })}</p>
              </div>
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">P&L</p>
